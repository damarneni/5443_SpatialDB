...
$sql = "
    (SELECT  
    AsWKB(EQ.SHAPE) AS wkb
    FROM earth_quakes AS EQ, state_borders AS SB
	WHERE CONTAINS (SB.SHAPE, EQ.SHAPE)  
	AND state = (SELECT state FROM `state_borders` WHERE CONTAINS (SHAPE, POINT({$_POST['lng']}, {$_POST['lat']})) ORDER BY state DESC LIMIT 1))
	union
	(SELECT asWKB(SHAPE) as wkb
		FROM state_borders A 
		WHERE CONTAINS(A.SHAPE,POINT({$_POST['lng']}, {$_POST['lat']})))
";
